...
ITEM_MAX_D_ = json_setting["model_param"][
    "max_d"
]  # diameter , NOTE: check this from object model?
...
ITEM_MODEL_FILE_PATH_ = os.path.join(
    CURR_DIR_, MODEL_FOLDER_NAME_, ITEM_NAME_, ITEM_MODEL_FILENAME_
)  # ./model/IPAGearShaft/IPAGearShaft.stl'
...
for cycle_idx in range(START_CYCLE_, MAX_CYCLE_ + 1):
    cycle_gt_matrix_path = os.path.join(
        GT_MATRIX_FOLDER_PATH_, "cycle_%04d" % cycle_idx
    )
